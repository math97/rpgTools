...
const buildEmbed = (): EmbedBuilder[] => {
  const classEmbed = buildEmbedClass(clerig)

  const levelEmbed = new EmbedBuilder()
    .setColor('Orange')
    .setTitle(`${clerig.className} Levels`)
    .setDescription(`Level features of the ${clerig.className} class`)
    .addFields(
      ...clerig.levels.map((level) => ({
...
export default {
  data: new SlashCommandBuilder()
    .setName('clerig')
    .setDescription(
      `Replies with data from ${clerig.className} class from D&D 5e!`,
    ),
  execute: async (
